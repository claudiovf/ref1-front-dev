...
const Weather: React.FC<Props> = ({nextRaceLoc, raceTime, sessionSelected}: Props) => {
    ...
    if (handleCountdown(raceTime.race).days > 5 ) return <Notif>*Weather forecast is available during race week</Notif>;
    ...
    useEffect(() => {
        ...
        if (weekendForecast) {
            const displayForecast = weekendForecast.find(forecast => forecast &&
                forecast.session === sessionSelected );
